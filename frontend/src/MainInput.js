...
const MainInput = ({setUsername,username,setUser,socket}) =>{
  ...
  },[setUser, setUsername, socket])
  const join = (e) =>{
    e.preventDefault();
    setDisable(true);
    if(username==="") {
      Swal.fire({text:"Enter some text please!",icon:'error'})
      setDisable(false);
      return;
    }
      ...
      <div className="MainInput__container">
       <form onSubmit={join} >
        <input 
          ...
          <button ref={buttonRef} onClick={join} style={{backgroundColor:disable?'lightgrey':'#27ae60'}} disabled={disable}>JOIN &#x2771;</button>
        </form>
      </div>
    );
}
